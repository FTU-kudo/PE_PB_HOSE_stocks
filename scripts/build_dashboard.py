...
<div class="page">
  ...
  <div class="g2">
    <div class="card"><div class="sec">📊 Sector Weighted P/E</div><div class="sec-sub">Capitalization-weighted</div><div style="position: relative; height: 450px; width: 100%;"><canvas id="chart-wpe"></canvas></div></div>
    <div class="card"><div class="sec">📊 Sector Weighted P/B</div><div class="sec-sub">Capitalization-weighted</div><div style="position: relative; height: 450px; width: 100%;"><canvas id="chart-wpb"></canvas></div></div>
  </div>

  <div class="g2">
    <div class="card"><div class="sec">📊 Sector Median P/E</div><div class="sec-sub">Green &lt;12 · Blue &lt;20 · Yellow &lt;30 · Red ≥30</div><div style="position: relative; height: 450px; width: 100%;"><canvas id="chart-pe"></canvas></div></div>
    <div class="card"><div class="sec">📊 Sector Median P/B</div><div class="sec-sub">Lower = cheaper vs book value</div><div style="position: relative; height: 450px; width: 100%;"><canvas id="chart-pb"></canvas></div></div>
  </div>

  <div class="card mb8">
    <div style="display:flex; justify-content:space-between; align-items:center;">
      <div class="sec" style="margin:0">📈 Sector Trend</div>
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(tG.length>0){
    ...
    let curMetric = 'pe';
    const metricConfig = {
      'wpe': { label: 'Weighted P/E', key: 'wpe' },
      'wpb': { label: 'Weighted P/B', key: 'wpb' },
      'pe': { label: 'Median P/E', key: 'pe' },
      'pb': { label: 'Median P/B', key: 'pb' }
    };
  ...
  $('#tbl').DataTable({
    ...
    initComplete: function () {
      this.api().columns([4, 5, 6]).every(function () {
        var column = this;
        var select = $('<select style="display:block;margin-top:6px;width:100%;padding:4px;border-radius:4px;background:var(--card2);color:var(--text);border:1px solid var(--border);font-weight:normal;font-size:0.8rem;"><option value="">All</option></select>')
