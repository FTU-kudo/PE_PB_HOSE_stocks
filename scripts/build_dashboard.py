...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Sector P/B chart
  const sPb  = D.sectors.filter(s => s.median_pb != null).slice().sort((a,b) => a.median_pb - b.median_pb);
  ...
  // ── Sector Weighted P/B chart
  const sWPb = D.sectors.filter(s => s.weighted_pb != null).slice().sort((a,b) => a.weighted_pb - b.weighted_pb);
  charts.wpb = new Chart(document.getElementById('chart-wpb'), {
    type: 'bar',
    data: {
      labels: sWPb.map(s => s.group),
      datasets: [{ data: sWPb.map(s => s.weighted_pb), backgroundColor: '#f472b6', borderRadius: 5 }],
    },
    options: {
      ...barBase(),
      plugins: {
