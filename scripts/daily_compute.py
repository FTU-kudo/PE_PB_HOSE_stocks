...
def register_vnstock() -> None:
    ...
    if not api_key:
        log.warning("VNSTOCK_API_KEY not set — running as Guest.")
        return
    try:
        from vnstock import Vnstock
        Vnstock().user.register(api_key=api_key)
        log.info("vnstock API key registered.")
    except Exception as exc:
        log.warning(f"Could not register API key: {exc}. Guest mode.")
...
def load_fundamentals() -> pd.DataFrame:
    ...
    log.info(f"Fundamentals loaded: {len(df)} tickers")
...
def fetch_close_prices(tickers: list[str]) -> pd.Series:
    ...
    ticker_col = next((c for c in board.columns if c.lower() in ("symbol", "ticker", "code")), board.columns[0])
    board = board.rename(columns={ticker_col: "ticker"})
    board["ticker"] = board["ticker"].str.upper()
    
    cols_l = {c.lower(): c for c in board.columns}
    for candidate in ("close_price", "match_price", "close", "matchedprice", "average_price", "reference_price"):
        if candidate in cols_l:
            price_col = cols_l[candidate]
            break
    else:
        price_col = next(c for c in board.columns if c != "ticker" and pd.api.types.is_numeric_dtype(board[c]))
...
def compute_pe_pb(close: pd.Series, fundamentals: pd.DataFrame) -> pd.DataFrame:
    ...
    # Vnstock 4.0 returns full VND prices, no need to multiply by 1000
...
def aggregate_sectors(snapshot: pd.DataFrame) -> pd.DataFrame:
    ...
    def _row(label, grp):
        ...
        pb = grp["pb"].dropna()
        return {
            "date": grp["date"].iloc[0], "group": label, "count": len(grp),
...
def _append_parquet(new_df: pd.DataFrame, path: str, date_col: str = "date") -> None:
    ...
    key = [date_col, "ticker"] if "ticker" in combined.columns else [date_col, "group"]
...
def update_history(snapshot: pd.DataFrame, sector_agg: pd.DataFrame) -> None:
    ...
    csv_path = Path(DAILY_DIR) / f"pe_pb_{str(date.today())}.csv"
    snapshot.to_csv(csv_path, index=False, sep=";")

def main():
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
    log.info(f"=== Daily PE/PB pipeline | {date.today()} ===")
    register_vnstock()
    fundamentals = load_fundamentals()
    close_prices = fetch_close_prices(fundamentals.index.tolist())
    snapshot     = compute_pe_pb(close_prices, fundamentals)
    sector_agg   = aggregate_sectors(snapshot)
    update_history(snapshot, sector_agg)
    log.info(f"=== Daily pipeline complete | {date.today()} ===")
